...
public class Sales {
	...
	private void entrance() {
		...
		this.display(this.title("직원코드"));
		oi.setEmployeesCode(this.userInput());
		...
		while(true) {
			...
			if(this.userInput().equals("Y")) {
				...
				//orders결제부분 ordercode,ecode,상품코드,수량
				clientData = "PA?orderCode="+si.getDate()+oi.getEmployeesCode()+
							"&employees"+oi.getEmployeesCode()+"&goodsCode="+oi.getGoodsCode()+"&quantity="+oi.getQuantity();
				serverData = ctl.entrance(clientData).split("&");
				
				//point결제부분  ordercode,membercode,point
				oi.setPoint(Double.parseDouble(String.format("%.2f%n",amount*10.0/100.0)));
				clientData =this.pointData(si.getStoreCode(), oi, si);
	...
	private String pointData(String storecode,OrderInfo oi,SalesInfo si) {
		/*주문코드 (OrderDate?orderCode=yyyyMMddHHmmssstorecode&point=243141*/
		SimpleDateFormat now = new SimpleDateFormat("yyyyMMddHHmmss");
		String clientData = "PA?orderCode="+si.getDate()+storecode+"&members=M01"+"&point="+oi.getPoint();
		return clientData;
	}
